  ...
  return (
    <View style={styles.wrap}>
      <View style={styles.nav}>
        ...
        <Text style={styles.title}>{formatMonthTitle(year, month, language)}</Text>
      ...
      <View style={styles.grid}>
        {grid.map((date) => {
          ...
          return (
            ...
            >
              <Text style={[styles.day, !inMonth && styles.dayOutside, selected && styles.daySelected]}>
                {date.getDate()}
              </Text>
              {status !== 'none' ? (
...
const styles = StyleSheet.create({
  ...
  navText: { color: colors.brand, fontSize: 28, textAlign: 'center' },
  title: { color: colors.ink, fontSize: 16, fontWeight: '700', textTransform: 'capitalize' },
  weekRow: { flexDirection: 'row' },
  weekday: { color: colors.muted, flex: 1, fontSize: 11, fontWeight: '600', textAlign: 'center' },
  ...
  cellSelected: { backgroundColor: 'rgba(142, 239, 180, 0.18)', borderRadius: 12 },
  cellToday: { borderColor: colors.brand, borderRadius: 12, borderWidth: 1 },
  cellOutside: { opacity: 0.4 },
  day: { color: colors.ink, fontSize: 14, fontWeight: '600' },
  dayOutside: { color: colors.muted },
  daySelected: { color: colors.brand },
