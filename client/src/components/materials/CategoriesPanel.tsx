...
export function CategoriesPanel() {
  ...
  return (
    <section className="rounded-3xl bg-panel shadow-sm ring-1 ring-line">
      <button
        type="button"
      ...
      >
        <div>
          <h2 className="text-sm font-semibold text-ink">{t('categories.title')}</h2>
          <p className="mt-0.5 text-xs text-muted">{t('materials.categoriesHint')}</p>
        </div>
        <ChevronDown
      ...
      {open ? (
        <div className="space-y-4 border-t border-line/70 px-4 py-4 sm:px-5">
              ...
              {!categories || categories.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {categories.map((category) => (
                    <li
                      key={category.id}
                      className="flex items-center justify-between gap-3 rounded-2xl bg-brand-50/30 px-4 py-3 ring-1 ring-line/70"
                    >
                      <div>
                        <p className="font-medium text-ink">{category.name}</p>
                        <p className="text-xs text-muted">
                          {t('categories.materialsCount', {
