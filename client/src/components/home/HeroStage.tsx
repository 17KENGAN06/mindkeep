...
/** Product atmosphere for the hero — full stage on desktop, compact strip on mobile. */
      ...
      <div className="relative mt-2 space-y-3 lg:hidden">
        <div className="grid gap-3 sm:grid-cols-3">
            ...
            <ul className="mt-3 space-y-2">
              {[t('home.heroVisual.tasksItem1'), t('home.heroVisual.tasksItem2')].map((item) => (
                <li key={item} className="flex items-start gap-2 text-sm text-ink">
                  <span className="mt-0.5 inline-flex h-4 w-4 shrink-0 items-center justify-center rounded border border-brand-500/50 bg-brand-500/15" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
          ...
          <article className="rounded-2xl border border-line/80 bg-panel/80 p-4 backdrop-blur-md">
            ...
            <p className="mt-2 rounded-xl border border-dashed border-brand-500/40 bg-brand-50/40 px-3 py-2 text-xs text-muted">
